...
//是否选择模糊化
checkedStatus = function (obj) {
	obj = 2;
	return obj;
}

//模糊化代码
phonehide = function (obj) {
	var phone = obj;
	var tel = "";
	if(phone.length == 11){
...
phonehideDeslit = function (obj) {
	var tel = "";
	var tel2 = obj.split(' ');
	var tel3 = " ";
	$.each(tel2,function (i,val) {
		if(val.length == 11){
			tel = val.substr(0, 3) + '****' + val.substr(7)+" ";
		}
		if(val.length > 3 && val.length <= 7){
			tel = val.substr(0, 3) + '****';
		}
		if(val.length == 12){
			tel = val.substr(0, 6) + '****' + val.substr(9)+" ";
		}
		if(val.length == 13){
			tel = val.substr(0, 6) + '****' + val.substr(10)+" ";
		}
		if(val.length == 14){
			tel = val.substr(0, 6) + '****' + val.substr(11)+" ";
		}
		if(val.length == 15){
			tel = val.substr(0, 7) + '****' + val.substr(12)+" ";
		}
		if(val.length == 16){
			tel = val.substr(0, 8) + '****' + val.substr(13)+" ";
		}
		tel3 += tel
	})
	return tel3;
